if (window.innerWidth <= 768) {
  const dopSlider = new Swiper('.dop__items-container', {
    spaceBetween: 10,
    freeMode: true,
    slidesPerView: 'auto',
    breakpoints: {
...
//Каунтер секции Product
const productCounterHtml = `
  <div class="info-box__counter">
    <button class="decrement">
      <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 11 11" fill="none">
        <circle cx="5.5" cy="5.5" r="5.5" fill="#817F7D"/>
        <rect x="2.44446" y="6.11108" width="1.22222" height="6.11111" rx="0.611111" transform="rotate(-90 2.44446 6.11108)" fill="white"/>
      </svg>
    </button>
    <span class="info-box__count">1</span>
    <button class="increment">
      <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 11 11" fill="none">
      <circle cx="5.5" cy="5.5" r="5.5" fill="#817F7D"/>
      <path fill-rule="evenodd" clip-rule="evenodd" d="M5.50001 2.44434C5.16251 2.44434 4.8889 2.71794 4.8889 3.05545V4.88878H3.05557C2.71806 4.88878 2.44446 5.16239 2.44446 5.49989C2.44446 5.8374 2.71806 6.111 3.05557 6.111H4.8889V7.94434C4.8889 8.28185 5.16251 8.55545 5.50001 8.55545C5.83752 8.55545 6.11112 8.28185 6.11112 7.94434V6.111H7.94446C8.28197 6.111 8.55557 5.8374 8.55557 5.49989C8.55557 5.16239 8.28197 4.88878 7.94446 4.88878H6.11112V3.05545C6.11112 2.71794 5.83752 2.44434 5.50001 2.44434Z" fill="white"/>
      </svg>
    </button>
  </div>
`;

const productBtnHtml = `
  <button class="info-box__btn1">Добавить в корзину</button>
`;

const productAddBtns = document.querySelectorAll('.info-box__btn1');

const productCounterShow = (event) => {
  //Получаю родительский элемент в который буду вшивать разметку counter
  const counterContainer = event.target.closest('.info-box__counter-container');
  counterContainer.innerHTML = productCounterHtml;
  const incrementBtn = counterContainer.querySelector('.increment');
  incrementBtn.onclick = handleProductIncrement;
  const decrementBtn = counterContainer.querySelector('.decrement');
  decrementBtn.onclick = handleProductDecrement;
}

const handleProductIncrement = (event) => {
  //Получаю родительский элемент counter
  const counterElement = event.target.closest('.info-box__counter');
  //Получаю элемент котрый отображает количество
  const count = counterElement.querySelector('.info-box__count');
  count.innerHTML = Number(count.innerHTML) + 1;
}
...
const handleProductDecrement = (event) => {
  ...
  if (Number(count.innerHTML) === 0) {
    const counterContainer = event.target.closest('.info-box__counter-container');
    counterContainer.innerHTML = productBtnHtml;
    counterContainer.querySelector('.info-box__btn1').onclick = productCounterShow;
  }
}

productAddBtns.forEach(btn => {
  btn.onclick = productCounterShow;
});


//Открытие окна с формой
const modalForm1 = document.querySelector('.modal-form-container');

window.addEventListener('click', (event) => {
  if (event.target.classList.contains('product-popup__item-btn')) {
    modalForm1.style.opacity = '1';
    modalForm1.style.visibility = 'visible';
  }
  
  else if (event.target.closest('.modal-closer')) {
    modalForm1.style.opacity = '0';
    modalForm1.style.visibility = 'hidden';
  }
