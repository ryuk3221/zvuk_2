...
//Скрипты для каунтера карзины
window.addEventListener('click', (event) => {
  //Нажали на +
  if (event.target.closest('.cart__item-increment')) {
    const cartItem = event.target.closest('.cart__item');
    const counter = event.target.closest('.cart__item-counter');
    const count = counter.querySelector('.cart__item-count');
    let priceElement = cartItem.querySelector('.cart__item-price');
    let price = parseInt(priceElement.innerHTML);
    count.innerHTML = parseInt(count.innerHTML) + 1;
    const itogElement = document.querySelector('.cart__info-value');
    itogElement.innerHTML = parseInt(itogElement.innerHTML) + price + ' ₽';
    showQuanyOfItems();
  }
  //Нажали на -
  else if (event.target.closest('.cart__item-decrement')) {
    const counter = event.target.closest('.cart__item-counter');
    const count = counter.querySelector('.cart__item-count');
    const cartItem = event.target.closest('.cart__item');
  ...
  //Нажали на удаление товара из корзины
  else if (event.target.closest('.cart__closer')) {
    const itogElement = document.querySelector('.cart__info-value');
    const cartItem = event.target.closest('.cart__item');
    let summOfCartItem = cartItem.querySelector('.cart__item-price').innerHTML;
    let countOfCartItem = cartItem.querySelector('.cart__item-count').innerHTML;
    countOfCartItem = parseInt(countOfCartItem);
    summOfCartItem = parseInt(summOfCartItem);
    itogElement.innerHTML = parseInt(itogElement.innerHTML) - summOfCartItem * countOfCartItem + ' ₽';
    cartItem.remove();
    showQuanyOfItems();
  }
});
...
const showQuanyOfItems = () => {
  const allItemCounts = document.querySelectorAll('.cart__item-count');
  let summ = 0;
  allItemCounts.forEach(el => {
    summ += Number(el.innerHTML);
  });
  document.querySelector('#cart__counts').innerHTML = summ;
}

//Открытие окна с формой
const modalForm2 = document.querySelector('.modal-form-container');

window.addEventListener('click', (event) => {
  if (event.target.classList.contains('cart__info-btn')) {
    modalForm2.style.opacity = '1';
    modalForm2.style.visibility = 'visible';
  }
  
  else if (event.target.closest('.modal-closer')) {
    modalForm2.style.opacity = '0';
    modalForm2.style.visibility = 'hidden';
  }
