...
const selectList = document.querySelector('.catalog__select-list');


const selectListShow = (event) => {
  selectList.classList.toggle('catalog__select-list--open');
};

selectList.addEventListener('click', (event) => {
  if (event.target.closest('.catalog__select-item') || event.target.closest('.select-item__dropdown-list')) {
...
window.addEventListener('click', (event) => {
  if (!event.target.closest('.catalog__select-wrapper') && selectList.classList.contains('catalog__select-list--open')) {
    selectListShow();
  }
});

selectHead.onclick = selectListShow;

const handleDropDownShow = (event) => {
  document.querySelectorAll('.catalog__dropdown-head').forEach(el => {
    if (el !== event.target) {
      const parent = el.closest('.catalog__menu-item');
  ...
  event.target.classList.toggle('drop-open');
  if (event.target.classList.contains('drop-open')) {
    const parent = event.target.closest('.catalog__menu-item');
    const ul = parent.querySelector('ul');
...
const dropHeads = document.querySelectorAll('.catalog__dropdown-head').forEach(el => {
  el.onclick = handleDropDownShow;
});



const selectItemHeads = document.querySelectorAll('.select-item__dropdown-head');

const showSelectItemMenu = (event) => {
  const parent = event.target.closest('.select-item__dropdown');
  parent.classList.toggle('select-item__dropdown-head--active');
  const ul = parent.querySelector('ul');
  if (parent.classList.contains('select-item__dropdown-head--active')) {
    ul.style.height = ul.scrollHeight + 'px';
  }
  else {
    ul.style.height = '0px';
...
selectItemHeads.forEach(item => {
  item.onclick = showSelectItemMenu;
});