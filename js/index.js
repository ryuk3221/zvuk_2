$(function() {
  ...
  // const btns = new Swiper('.propositions__btns', {
  //   spaceBetween: 10,
  //   freeMode: true,
  //   slidesPerView: 3,
  // });
  ...
  $('.small-slider').slick({
    ...
    asNavFor: '.big-slider',
    focusOnSelect: true,
    nextArrow: '<button type="button" class="slick-next"><img src="images/slider-banner/arrow-right.svg"></button>',
    prevArrow: '<button type="button" class="slick-prev"><img src="images/slider-banner/arrow-right.svg"></button>',
    autoplay: true,
    swipeToSlide: true,
    touchThreshold: 30,
    autoplaySpeed: 3000
  ...
  const propositionsSlider = new Swiper('.propositions__inner', {
    spaceBetween: 10,
    freeMode: true,
    slidesPerView: 'auto',
    navigation: {
      nextEl: '.swiper-button-next-props',
      prevEl: '.swiper-button-prev-props',
    },
    breakpoints: {
      768: {
        spaceBetween: 20,
  ...
  //Скрипты для аккордиона (вопросы и ответы)
  //Получаю все верхушки вопроса
  const questionHeaders = document.querySelectorAll('.questions__item-head');
  //Получаю все родительские элементы вопроса
  const allQuestionItems = document.querySelectorAll('.questions__item');

  const questionTextShow = (event) => {
    //Получаю родительский элемент вопроса по которому кликнули
    const thisQuestionParent = event.target.closest('.questions__item');
    //Пробегаюсь по всем родительским элементам вопроса
    allQuestionItems.forEach(item => {
      //Получаю выпадающий текст вопроса (ответ)
      const response = item.querySelector('p');
      //Удаляю активный класс у всех тех элементов которые не равняются тому по которому кликнули
      if (item != thisQuestionParent) {
        item.classList.remove('questions__item--open');
        response.style.height = '0';
        response.style.marginBottom = '0';
      }
      else {
        ...
        if (item.classList.contains('questions__item--open')) {
          ...
          response.style.marginBottom = '22px';
        }
        else {
          response.style.height = '0';
          response.style.marginBottom = '0';
