...
const linksHeads = document.querySelectorAll('.category-links__item-head');
const accordItems = document.querySelectorAll('.category-links__item');

const dropDownMenuShow = (event) => {
  if (window.innerWidth < 1260) {
    //Получаю родителький элемент выпадающего меню по которому кликнули
    const accrodItemCurrent = event.target.closest('.category-links__item');
    accordItems.forEach(el => {
      if (el != accrodItemCurrent) {
        el.classList.remove('category-links__item--active');
        const dropdownList = el.querySelector('ul');
        dropdownList.style.height = '0';
        dropdownList.style.marginTop = '0';
      }
    
    });
    accrodItemCurrent.classList.toggle('category-links__item--active');
    if (accrodItemCurrent.classList.contains('category-links__item--active')) {
      const dropdownList = accrodItemCurrent.querySelector('ul');
      dropdownList.style.height = dropdownList.scrollHeight + 'px';
      dropdownList.style.marginTop = '17px';
    }
    else {
      const dropdownList = accrodItemCurrent.querySelector('ul');
      dropdownList.style.height = '0';
...
linksHeads.forEach(el => {
  el.onclick = dropDownMenuShow;
});
